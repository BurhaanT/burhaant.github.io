...
Keep calm and carry on! This seems to be nothing more than marketing that's meant to make AAD more appealing to developers. As far as I can tell, there's nothing new insofar as AAD is concerned but it does provide some handy SDK's and libraries that help leverage AAD within your application.
...
Okay, so using AAD I can usure that only particular _people_ can access a resoure (Authentication) and can only perform actions they are allowed to (Authorisation).
...
Identity and access management can be a complex domain and while I haven't setup something like this from scratch myself, I can't say I'd want to. I enjoy building apps; I enjoy writing code. Sometimes my apps need to be secure. That security needs to be reliable, always available and setup to be compliant and secure. I want to be able to incorporate these elements into my application as quickly as possible with the least amount of friction.

I'm sure the other cloud providers are equally as capable but I am more familiar with the Microsoft stack so I'll just go with that - justification enough for me.

I need to specifically mentioned [Yasers blog](https://mehraban.com.au){:taget="_blank"} here - An excellent resource and I've probably used it a few times when I've got stuck. In particular, he's got a great post on [using Azure B2C as your identity manager](https://mehraban.com.au/2017/08/16/using-azure-b2c-identity-manager-part-1/){:target="_blank"}. 

## The Outcome

I'd like to start here. A contrived example will help me get to an end goal that makes sense. 
My purpose is to create a simple API that requires Authentication. I'd like my API consumer to be able to provide a user name and password that's authenticated against AAD. AAD should then return a token and that token is passed to my API in the header (i.e. Bearer Authentication) and verifiable by my API. If the token in valid, allow the consumer to interact with my API otherwise, deny!
...
Following the instructions on setting up AAD makes it look simple. Before moving on, I'd like to make sure that bit works as expected so I dont (later) start hunting down phantom authentication errors in code.
As such, at this stage I want to test independant of code and Postman is a useful otion.
...
I need to get an API project up and running quickly so I'll run `dotnet new webapi`. This gives me a base from which to start. Done and dusted...ready to go!
